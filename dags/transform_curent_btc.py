...
in_btc_dataset = Table(c.HISTORY_CURRENT_BTC_TABLE_NAME, conn_id=gv.CONN_ID_DUCKDB)
...
def transform_btc_data():
    ...
    create_btc_reporting_table(
        in_btc=Table(name=c.HISTORY_CURRENT_BTC_TABLE_NAME, conn_id=gv.CONN_ID_DUCKDB),
        output_table=Table(name=c.REPORT_HISTORICAL_BTC_TABLE_NAME, conn_id=gv.CONN_ID_DUCKDB),
    )
